...
[1] 선별·그룹핑    감사 브리프(인모델: 목적·렌즈 우선순위·고위험 영역)
    (모델+스크립트)  + select_targets(core/low/exclude) + 분류 검토(인모델)
                    + group_by_lines(라인수+import 응집, 예산 10K)  ──▶ groups.json
                                                                       │ 병렬 위임 (동시 4~6)
[2] 적대적 탐지    그룹별 헌터 서브에이전트                             ▼
    (모델)          · 전역 읽기 / 그룹 한정 보고
                    · 비대칭 기록 · coverage 증거                   ──▶ defects/<gid>.json
                                                                       │
[2.5] 보강 패스    · cross_refs 힌트 라우팅 → 표적 sweep                ▼
    (모델+스크립트)  · 고위험 그룹 critical 전용 2차 헌터            ──▶ defects/<gid>.json 갱신
                                                                       │
...
프롬프트 골격은 [references/hunt-agent.md](../../references/hunt-agent.md) —
primary/sweep/second_pass 세 모드가 공용하며 모드 스위치만 갈린다.

- **읽기 범위 ≠ 보고 범위 (핵심 규칙).** 읽기는 저장소 전체 자유(상류 검증 여부 등
  전역 맥락 확인 → 오탐 감소; 단 필요 기반 선택 열람), 보고는 결함 라인이 자기 그룹
  파일 안에 있는 것만(결함 라인의 소유자가 유일 → 중복·월경 보고 없음).
- **타 그룹 결함 징후는 `cross_refs` 힌트로만** 남긴다(`category` 필수 — 커버 판정에
  사용). 힌트는 사장되지 않고 Stage 2.5가 소비한다.
...
1. **힌트 라우팅 → 표적 sweep.** `validate_output.py route-hints`(결정적 작업)가 전체
   `cross_refs`를 소유 그룹으로 라우팅하고, 기존 findings가 커버하는 힌트와 exclude
   대상 힌트를 걸러 `hints/<gid>.json`을 만든다. 커버 판정은 **위치 중첩 + 동일
   `category`** — 같은 라인에 이종 결함이 공존할 수 있어 위치만으로 폐기하면 미탐
   채널이 된다. 남은 힌트 그룹마다 **sweep 헌터**가 힌트 지점만 집중 조사해
   `defects/<gid>.sweep.json`(ID 접두사 `w`)에 독립 기록한다. 검증 불능(failed)
   그룹으로 라우팅된 힌트도 sweep은 수행한다(저비용으로 감사 공백을 부분 보전).
2. **고위험 critical 전용 2차 패스.** `high_risk: true` 그룹에 한해 "critical만
...
결과가 병합 전 발견을 전부 보존하는지)를 수행한다.

### Stage 3 — 적대적 검증

발견이 있는 그룹마다 **새 컨텍스트** 검증자를 위임한다(헌터와 컨텍스트 비공유).
프롬프트 골격은 [references/verify-agent.md](../../references/verify-agent.md).
발견이 많으면 묶음 분할해 각자 `verified/<gid>.batch-N.json`에 쓰고 스크립트가
병합한다(단일 작성자 원칙 — sweep·2차 패스와 같은 패턴).
...
- **에이전트 타입**: 파일 쓰기가 가능한 범용 서브에이전트(읽기 전용 탐색 타입은 산출
  JSON을 못 씀). 모델은 지정하지 않고 상속.
- **재시도 — 3가지 실패 양식을 구분**:
...
| 저장소 내 지시문(프롬프트 인젝션) 순응 | hunt/verify 템플릿의 데이터 취급 원칙 + 의심 신호 격상 |
...
```
SKILL.md                     # 오케스트레이터: 4단계 절차, 자연어 인터페이스, 병렬·재시도 정책, 재개
references/
  hunt-agent.md              # 탐지 프롬프트 골격 (primary/sweep/second_pass 모드 스위치)
  verify-agent.md            # 검증 프롬프트 골격 (anti-anchoring, 룰브릭, 차등 검증)
...
scripts/                     # 표준 라이브러리만 사용, 단독 실행 CLI
  ...
  test_scripts.py            # 단위 테스트 34종 (파서·그룹핑·검증·병합·보고서·저하 감지)
```

검증 명령: `python3 scripts/test_scripts.py` (외부 의존성 없음),
`python3 -m py_compile scripts/*.py`. 설치는 `~/.claude/skills/deep-code-audit`이 이
레포로의 **심링크**라 별도 동기화가 필요 없다.
